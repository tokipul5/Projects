...
class Game {
    ...
    /** Set both players to auto. */
    private void aa() {
        _white = _autoPlayerTemplate.create(WP, this);
        _black = _autoPlayerTemplate.create(BP, this);
    }

    /** Set white auto and black manual. */
    private void am() {
        _white = _autoPlayerTemplate.create(WP, this);
        _black = _manualPlayerTemplate.create(BP, this);
    }

    /** Set white manual and black auto. */
    private void ma() {
        _white = _manualPlayerTemplate.create(WP, this);
        _black = _autoPlayerTemplate.create(BP, this);
    }

    /** Set both players to manual. */
    private void mm() {
        _white = _manualPlayerTemplate.create(WP, this);
        _black = _manualPlayerTemplate.create(BP, this);
    }

    /** Go back to two steps before. */
    private void undo() {
        if (!_undoStack.isEmpty() && _undoStack.size() > 1) {
            Board temp = _undoStack.get(_undoStack.size() - 2);
            _undoStack.remove(_undoStack.size() - 1);
            _undoStack.remove(_undoStack.size() - 1);
            _board = new Board(temp);
        }
    }
    ...
    private boolean processMove(String line) {
        ...
        if (move == null) {
            ...
            _undoStack.add(new Board(_board));
