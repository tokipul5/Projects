    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                if self.ant.can_contain(insect): #bodyguard first
                    self.ant.contain_ant(insect)
                elif insect.can_contain(self.ant): #non-bodyguard first
...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        if not self.is_scared and self.num_scared != 0:
            destination = self.place.entrance
            self.num_scared -= 1
            if self.num_scared == 0:
                self.is_scared = True
        "*** YOUR CODE HERE ***"
        # END EC
        if self.blocked():
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        #Find a place of the ant
        while curr != beehive:
            if curr.ant:
                break
            curr = curr.entrance

        #Find a place of the bee and check the range
        while curr != beehive:
            if curr.bees and distance <= self.max_range and distance >= self.min_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        Insect.reduce_armor(self, amount) #remove the insect itself
        if self.armor <= 0:
            self.damage += amount
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        "*** YOUR CODE HERE ***"
        if self.digesting == 0:
            self.eat_bee(ThrowerAnt.nearest_bee(self, colony.beehive))
        else:
            self.digesting -= 1
...
class BodyguardAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        if self.contained_ant is not other and other.is_container == False and self.contained_ant == None:
            return True
        else:
            return False
        # END Problem 9
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        ...
        FireAnt.reduce_armor(self, self.damage)
        # END Problem 10
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.is_queen == True:
            ThrowerAnt.action(self.queen, colony)
            curr = self.place
            while curr != None:
                curr = curr.exit
                if curr != None and curr.ant != None:
                    if  isinstance(curr.ant, BodyguardAnt):
                        if curr.ant.contained_ant == None and curr.ant.damage_doubled == False:
                            curr.ant.damage *= 2
                            curr.ant.damage_doubled = True
                        elif curr.ant.contained_ant.damage_doubled == False:
                            curr.ant.contained_ant.damage *= 2
                            curr.ant.contained_ant.damage_doubled = True
                    if curr.ant.damage_doubled == False:
                        curr.ant.damage *= 2
                    curr.ant.damage_doubled = True
        else:
...
def apply_effect(effect, bee, duration):
    ...
    ori_effect = bee.action
    def new_action(colony):
        nonlocal turns
        if turns == 0:
            return ori_effect(colony)
        else:
            turns -= 1
            return new_effect(colony)
